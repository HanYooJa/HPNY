...
export default function Navbar(){
  ...
  >
    ...
    <div className="w-full sm:w-[280px] border py-1.5 border-gray-200 rounded-full shadow hover:shadow-lg cursor-pointer flex justify-between pl-6 pr-2">
      ...
      <div className="sm:w-[340px] cursor-pointer w-full relative">
        <div className="flex justify-center gap-7 h-14 text-center items-center">
          ...
          <button type="button" className="text-gray-700" onClick={() => window.alert('서비스 준비중입니다.')}>
            체험
          </button>
          <button type="button" className="text-gray-700" onClick={() => window.alert('서비스 준비중입니다.')}>
            온라인 체험
          </button>
          <button type="button" className="underline underline-offset-8 text-gray-500 hover:text-black" onClick={() => setShowFilter(false)}>
            필터 닫기
          </button>
        </div>
        <div className="w-[90%] sm:max-w-3xl flex flex-col sm:flex-row border border-gray-200 rounded-lg py-4 sm:py-2 sm:rounded-full shadow-sm bg-white hover:shadow-lg cursor-pointer justify-between fixed top-20 inset-x-0 mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-4 w-full relative sm:pl-2">
            <button type="button" onClick={() => setDetailFilter('location')} className={cn("font-semibold text-xs rounded-full hover:bg-gray-100 py-3 px-6 text-left", {
            ...
            <button type="button" onClick={() => setDetailFilter('checkIn')} className={cn("font-semibold text-xs rounded-full hover:bg-gray-100 py-3 px-6 text-left", {
            ...
            <button type="button" onClick={() => setDetailFilter('checkOut')} className={cn("font-semibold text-xs rounded-full hover:bg-gray-100 py-3 px-6 text-left", {
            ...
            <button type="button" onClick={() => setDetailFilter('guest')} className={cn("font-semibold text-xs rounded-full hover:bg-gray-100 py-3 px-6 text-left", {
    ...
    <div className="grow basis-0 hidden md:flex gap-4 align-middle justify-end relative">
      <button type="button" className="font-semibold text-sm my-auto px-4 py-3 rounded-full hover:bg-gray-50">
        당신의 공간을 등록해주세요
        </button>
