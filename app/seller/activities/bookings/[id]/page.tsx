...
export default function SellerActivityBookingDetailPage({
  params,
}: {
  params: { id: string }
}) {
    ...
    <div className="max-w-5xl mx-auto px-4 pt-10 pb-20">
      ...
      <div className="rounded-md border border-gray-300 p-6 mt-10">
        <section className="flex border-b gap-4 pb-6">
          <Image
            src={booking?.activity?.images?.[0] || "/images/default-image.png"}
            width={100}
            height={100}
            alt="체험활동 이미지"
          ...
          <div className="flex flex-col justify-between">
            <h1 className="text-sm">{booking?.activity?.title}</h1>
            <p className="text-xs text-gray-500">
              {booking?.activity?.category} |{" "}
              {booking?.activity?.price?.toLocaleString()}원
            </p>
            <p className="text-xs text-gray-500">
              후기 {booking?.activity?.comments?.length ?? 0}개
            </p>
          </div>
        </section>
        ...
        <section className="flex flex-col gap-4 pb-6">
          ...
          <div className="flex justify-between items-center">
            <h3 className="font-bold">활동 일정</h3>
